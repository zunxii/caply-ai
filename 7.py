...
# === Load Inputs ===
chunks = json.load(open("output/chunks.json", encoding="utf-8"))
style_seq = json.load(open("output/style_sequence_by_frame.json", encoding="utf-8"))
templates = json.load(open("output/templates.json", encoding="utf-8"))

# === Template lookup for quick access
...
for chunk in chunks:
    ...
    if not matched:
        print(f" Skipping chunk (no matching frame): {chunk['chunk_text']}")
        continue
    ...
    log_entry = f"Chunk: \"{chunk['chunk_text']}\"\nMatched Frame: {frame_name} at {data['time_ms']} ms\nAssigned Styles:\n"
...
final_output = ass_header + "\n" + "\n".join(ass_lines)
Path("output").mkdir(exist_ok=True)
Path("output/styled_output.ass").write_text(final_output, encoding="utf-8")
Path("output/logs.txt").write_text("\n".join(log_lines), encoding="utf-8")

print("✅ Styled .ASS file saved to: output/styled_output.ass")
print("📝 Logs saved to: output/logs.txt")
